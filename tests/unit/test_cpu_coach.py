...
def test_trend_tracker_records_summaries():
    ...
    s = PossessionSummary(
        side=Side.HOME, scored=True, points=3, zone="three", turnover=False, foul=False
    )
    t.record(s)
...
def test_trend_opponent_scored_count():
    ...
    for _ in range(3):
        t.record(PossessionSummary(
            side=Side.HOME, scored=False, points=0,
            zone="mid", turnover=True, foul=False,
        ))
...
def test_aggressive_personality_high_pace():
    assert (
        assign_personality(pace=74.0, off_tov_pct=0.18, def_efg=0.45)
        is CpuPersonality.AGGRESSIVE
    )


def test_aggressive_personality_low_tov():
    assert (
        assign_personality(pace=68.0, off_tov_pct=0.12, def_efg=0.45)
        is CpuPersonality.AGGRESSIVE
    )
...
def test_conservative_personality_slow_pace():
    assert (
        assign_personality(pace=62.0, off_tov_pct=0.18, def_efg=0.45)
        is CpuPersonality.CONSERVATIVE
    )


def test_conservative_personality_strong_defense():
    assert (
        assign_personality(pace=68.0, off_tov_pct=0.18, def_efg=0.38)
        is CpuPersonality.CONSERVATIVE
    )
...
def test_zone_on_spacer_heavy_lineup():
    ...
    result = coach.should_switch_scheme(
        ...
        opp_lineup_archetypes=[
            "floor_spacer", "versatile_wing", "floor_spacer", "default", "default"
        ],
...
def test_trend_tracker_points_by_player():
    ...
    t.record(PossessionSummary(
        side=Side.HOME, scored=True, points=3,
        zone="three", turnover=False, foul=False, player="Smith",
    ))
    t.record(PossessionSummary(
        side=Side.HOME, scored=True, points=2,
        zone="rim", turnover=False, foul=False, player="Smith",
    ))
    t.record(PossessionSummary(
        side=Side.HOME, scored=True, points=2,
        zone="mid", turnover=False, foul=False, player="Jones",
    ))
...
def test_foul_trouble_first_half_role_player_2_fouls():
    """Unified rule: role players (importance rank >= 2) are pulled at
    2 fouls in the first half, matching fatigue.check_substitutions."""
    coach = _make_coach()
    # Player 0 is the clear star; the rest are role players.
...
def test_hot_hand_no_veto_below_threshold():
    ...
    coach.trend.record(PossessionSummary(
        side=Side.AWAY, scored=True, points=2,
        zone="rim", turnover=False, foul=False, player="Star",
    ))
    coach.trend.record(PossessionSummary(
        side=Side.AWAY, scored=True, points=2,
